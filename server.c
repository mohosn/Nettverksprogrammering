...
char *mimeType(char *path)
{
    ...
    //for content type from mime.types

    //no extension or invaild extension--> NULL
    //asis --> asis
    // vaild extension --> valid content-type
    ...
    //path = /index   --> no extension  --> fileextension = NULL

    //fileextentsion will be --> .asis
    ...
    // pointing .html
    fileExtension++; // pointing html     asis
    ...
    {                                          // reading line by line from mime.types
        ...
        {                        // checking if file extension exists as a substring in the line
            ...
            storeToken = strtok(NULL, "\t"); //will creat two substring one for content-type and one for extensions
            ...
            {   
                ...
                { // extracting all token
                    if (!strcmp(storeToken, fileExtension)) //jpeg == jpg ? no
...
int main()
{
    ...
    while (1)
    {
        ...
        if (!fork()) // creating new process for each client request by creating a child process
        {
            ...
            // localhost/index.asis

            //filepathcopy = /index.asis

            contentType = mimeType(filePathCopy); // calling mimeType to check content-type
            fprintf(stderr, "%s\n", filePathCopy);
            // localhost/
            if (!strcmp(filePath, "/")) //localhost --> index.html
            {
                filePath = "/index.html";
            ...
            if (contentType == NULL)  //either no extension or invalid extension
            {
                ...
                close(clientSocket);
            }else if (access(filePath, F_OK))  // either asis or valid content-type
            { // checking if the filepath exists in the system
                ...
                //not setting header just opening the asis file and sending back to the client
                fp = fopen(filePath, "r");
            ...
            {
                sprintf(httpOk, "HTTP/1.1 200 OK\nContent-Type: %s\r\n\r\n", contentType); //setting the content type
